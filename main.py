...
def main():
    ...
    parser.add_argument('--cv-folds', type=int, default=1,
                        help='Number of StratifiedKFold folds (1 disables CV)')
    ...
    for tr_ratio in train_ratios:
        ...
        # Cross-validation on training portion if requested
        if args.cv_folds > 1 and args.model == 'igann':
            from sklearn.model_selection import StratifiedKFold
            cv = StratifiedKFold(n_splits=args.cv_folds, shuffle=True, random_state=args.seed)
            fold_aucs = []
            for fold, (tr_idx, val_idx) in enumerate(cv.split(X_train_full, y_train_full), 1):
                X_tr_raw, X_val_raw = X_train_full[tr_idx], X_train_full[val_idx]
                ...
                _, m = train_igann(
                    X_trn, y_tr_bal, X_valn, y_val, X_valn, y_val,
                ...
                print(f"Fold {fold}/{args.cv_folds} val_auc={m['auc']:.4f}")
            print(f"CV mean AUC: {np.mean(fold_aucs):.4f}")

        # Hold-out validation for final training
