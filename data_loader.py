...
def load_sgcc_csv(data_dir: str) -> tuple[np.ndarray, np.ndarray]:
    feat_path = os.path.join(data_dir, 'feature.csv')
    lab_path = os.path.join(data_dir, 'label.csv')
    X = pd.read_csv(feat_path, header=None).values
    y_df = pd.read_csv(lab_path, header=None)
